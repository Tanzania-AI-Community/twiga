...
async def handle_command_message(user: User, message: Message) -> JSONResponse:  # type: ignore
    ...
    if message.content == "settings":
        ...
        await whatsapp_client.send_message(
            generate_payload(user.wa_id, response_text, options)
        )
    else:
        response_text = "Sorry, I don't understand that command."
        await whatsapp_client.send_message(generate_payload(user.wa_id, response_text))
    return JSONResponse(
        content={"status": "ok"},
        status_code=200,
    )
...
async def handle_chat_message(user: User, user_message: Message) -> JSONResponse:
    ...
    if llm_responses:
        ...
        await whatsapp_client.send_message(
            generate_payload(user.wa_id, llm_responses[-1].content)
        )
    else:
        logger.error("No responses generated by LLM")
        err_message = "Sorry, something went wrong on my end. Please try again later."
        await whatsapp_client.send_message(generate_payload(user.wa_id, err_message))
