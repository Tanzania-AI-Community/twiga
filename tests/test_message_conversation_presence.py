...
@pytest.mark.asyncio
async def test_request_service_marks_inbound_user_message_as_present_in_conversation() -> (
    None
):
    user = User(
        id=7,
...
@pytest.mark.asyncio
async def test_tool_call_notification_persists_message_present_in_conversation() -> (
    None
):
    client = DummyClient()
    ...
    persisted_kwargs = mock_create_new_message.await_args.kwargs
    assert persisted_kwargs["user_id"] == user.id
    assert persisted_kwargs["role"] == enums.MessageRole.assistant
    assert persisted_kwargs["content"] == "Using search_knowledge..."
...
async def test_handle_chat_message_marks_final_response_as_present_in_conversation() -> (
    ...
    with (
        ...
        patch(
            "app.services.messaging_service.db.create_new_messages", AsyncMock()
        ) as mock_create_messages,
        patch(
            "app.services.messaging_service.whatsapp_client.send_message", AsyncMock()
        ) as mock_send_message,
